...
export const useDriverDataManagement = () => {
  ...
  const fetchSingleDriverData = async (driverNumber: number): Promise<void> => {
    ...
    try {
        ...
        driverNumber,
        {
          onProgress: (loaded, total, info) => {
            ...
            if (
              ...
              raceStore.selectedSession?.session_key === currentSessionKey
            ) {
              raceStore.setLoadingProgress({
                loaded,
                total,
                isLoading: true,
                progressInfo: info,
      ...
      if (
        ...
        console.error(
          `Error fetching driver ${driverNumber} location data:`,
          error
        );
  ...
  const handleFetchDriver = (driverNumber: number) => {
    ...
    if (
      raceStore.loadedDrivers.has(driverNumber) ||
      raceStore.fetchingDrivers.has(driverNumber)
    ) {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (
      driverData.length > 0 &&
      raceStore.selectedSession &&
      currentSessionKey !== lastSessionKeyRef.current
    ) {
