...
const LoadingProgressBar = ({ loadingProgress }: { loadingProgress: LoadingProgress }) => (
    <div className="space-y-2">
...
const SpeedControls = ({
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            {[0.0185, 0.037, 0.074, 0.111, 0.185, 0.37, 0.555, 0.925, 1.85].map((speed) => (
                <Button
                    key={speed}
                    variant={animationState.speed === speed ? "default" : "outline"}
                    size="sm"
                    onClick={() => onSpeedChange(speed)}
                    className="w-16 bg-zinc-800 border-zinc-700 hover:bg-zinc-700"
                >
...
const SessionInfo = ({ sessionData }: { sessionData: { session_name?: string; date_start?: string; location?: string; } }) => {
